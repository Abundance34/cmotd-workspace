...
PROCUREFLOW_UPLOAD_DIR=data/attachments
...
## 2026 Cloud SQL and E2E Workflow Update

This build now supports a production PostgreSQL backend with pooled Cloud SQL Unix-socket connections while retaining SQLite only for local development/recovery. It also adds:

- automatic password-change session revocation/rotation and dashboard return;
- vendor-owned quote totals and line-item pricing;
- one authoritative light-only UI theme with stable sidebar contrast;
- immutable pre-payment approval rescission;
- separate Create Request and View Requests blocks without replacing Request Register;
- selected vendor/payee/approval relationships flowing into Finance;
- structured Payee & Bank Details instead of raw JSON for authorised roles;
- exact Internet Bank Transfer and Physical Bank Transfer values;
- print/PDF Approved Payment Instruction;
- idempotent complete-chain payment notifications;
- file-only Proof of Payment and Vendor Receipt uploads with persisted OCR review;
- one atomic Mark Completed action that also closes and archives the record;
- restart-safe SQLite-to-PostgreSQL migration, persistent-document transfer/path rewriting, verification, health-check, and rollback tooling.
...
- `docs/MIGRATION_REPORT_FORMAT.md`
- `docs/REGRESSION_TEST_REPORT.md`
- `docs/POSTGRESQL_STABILIZATION_REPORT.md`
- `docs/POSTGRESQL_COMPATIBILITY_AUDIT.md`
