...
from core.db_backend import get_postgres_connection, is_postgres

BASE_DIR = Path(__file__).resolve().parents[1]
DATA_DIR = Path(os.environ.get("PROCUREFLOW_DATA_DIR", BASE_DIR / "data")).expanduser().resolve()
DB_PATH = Path(os.environ.get("PROCUREFLOW_SQLITE_PATH", DATA_DIR / "procureflow_workspace.db")).expanduser().resolve()
ATTACHMENT_DIR = Path(os.environ.get("PROCUREFLOW_UPLOAD_DIR", DATA_DIR / "attachments")).expanduser().resolve()
IMPORT_DIR = Path(os.environ.get("PROCUREFLOW_IMPORT_DIR", DATA_DIR / "imports")).expanduser().resolve()
BACKUP_DIR = Path(os.environ.get("PROCUREFLOW_BACKUP_DIR", DATA_DIR / "backups")).expanduser().resolve()
...
def _seed_hash_password(password: str) -> str:
    ...
    if password is None:
        password = ""
    # Tests create many isolated copies of the seeded workspace.  Permit a
    # deliberately explicit test-only cost reduction so the suite remains fast
    # without weakening production or user-created password hashes.
    iterations = 1_000 if os.environ.get("PROCUREFLOW_FAST_TEST_SEEDS") == "1" else 260_000
    salt = os.urandom(16)
...
def get_conn():
    ...
    # journal_mode is persistent at database-file level. Reissuing it on every
    # short-lived Streamlit connection can block behind another reader and make
    # routine schema checks appear hung. Configure it once per process/path.
    db_key = str(DB_PATH)
    if db_key not in _SQLITE_WAL_CONFIGURED_PATHS:
...
def insert_and_get_id(conn, query: str, params: Iterable[Any] = ()) -> int:
    """Insert on an existing transaction and return the generated integer ID."""
    cur = conn.cursor()
    if is_postgres():
        ...
        row = cur.fetchone()
        if not row:
            raise RuntimeError("Insert completed without returning a primary key.")
        return int(row[0])
    cur.execute(query, tuple(params))
...
def df_query(query: str, params: Iterable[Any] = ()) -> pd.DataFrame:
    ...
    try:
        if is_postgres():
            ...
            return pd.DataFrame([tuple(row.values()) for row in rows], columns=columns)
...
def init_db():
    ...
    if _DB_INIT_DONE and (is_postgres() or DB_PATH.exists()):
        return
    if is_postgres():
        from core.postgres_schema import initialize_postgres_database
...
def _queue_notification_channel(notification_id: int | None, channel: str, user_id: int | None, role: str | None, status: str = "Queued", error: str | None = None):
    # The application startup path initializes every notification table before
    # any workflow action can run. Repeating the full phase-two schema audit for
    # each recipient caused multi-second notification fan-out and test-suite
    # stalls. Standalone callers that bypass init_db() still retain the safe
    # fallback below.
...
def _recent_duplicate_notification(target_uid, role, title, message, entity_type, entity_id) -> bool:
    ...
    rows = run_query(
        ...
        (
            ...
            entity_type,
            entity_id,
            (datetime.now(timezone.utc) - timedelta(seconds=15)).isoformat(timespec="seconds"),
        ),
        fetch=True,
    )
    return bool(rows)
...
def create_notification(
    ...
    """
    # init_db() (or initialize_postgres_database()) performs these schema
    # checks once at process startup. Avoid re-running dozens of DDL and
    # information-schema queries for every workflow participant. This retains
    # a guarded fallback for direct library use before normal application boot.
    if not _DB_INIT_DONE:
        ensure_phase2_schema()
        ensure_finance_document_schema()
        try:
    ...
    for target_uid in target_user_ids:
        if dedupe_key:
            duplicate = run_query(
                "SELECT id FROM notifications WHERE dedupe_key=? AND COALESCE(user_id,-1)=COALESCE(?, -1) LIMIT 1",
                (dedupe_key, target_uid),
                fetch=True,
            )
            if duplicate:
                continue
        ...
        if "in_app" in channels_local or "all" in channels_local or (not channels_local and not pref):
            ...
            if not _recent_duplicate_notification(target_uid, role_target, title, message, entity_type, entity_id):
                notification_id = run_insert(
                    """
                    INSERT INTO notifications (user_id, role, title, message, entity_type, entity_id, is_read, popup_shown, importance, delivery_channel, push_sent, email_sent, action_url, action_label, section_target, dedupe_key, created_at)
                    VALUES (?, ?, ?, ?, ?, ?, 0, 0, ?, ?, 0, 0, ?, ?, ?, ?, ?)
                    """,
                    (target_uid, role_target, title, message, entity_type, entity_id, importance, delivery_channel or "in_app", action_url, action_label, section_target, dedupe_key, now_iso()),
                )
                created_ids.append(notification_id)
...
def _create_generic_audit_trigger(table: str, entity_type: str, reference_expr: str, id_expr: str = "NEW.id") -> None:
    ...
    """
    if is_postgres():
        # PostgreSQL trigger syntax is installed by numbered migrations.
        return
...
def ensure_audit_hardening_schema() -> None:
    ...
    for sql in [
        ...
        if is_postgres() and "CREATE TRIGGER" in sql.upper():
            continue
        try:
            run_query(sql)
        except Exception:
...
# ---------------- 2026 E2E workflow enhancement schema ----------------

def ensure_e2e_update_schema() -> None:
    """Add restart-safe columns/tables for the approved E2E improvements.

    SQLite remains available for local development. Production PostgreSQL uses
    the numbered SQL migrations in ``migrations/postgresql`` with equivalent
    types, constraints and indexes.
    """
    migrations: dict[str, list[tuple[str, str]]] = {
        "vendor_quotes": [
            ("request_id", "request_id INTEGER"),
            ("currency", "currency TEXT DEFAULT 'NGN'"),
            ("quote_date", "quote_date TEXT"),
            ("supply_description", "supply_description TEXT"),
            ("quote_document_path", "quote_document_path TEXT"),
            ("quote_document_hash", "quote_document_hash TEXT"),
            ("quotation_total", "quotation_total REAL"),
            ("is_selected", "is_selected INTEGER DEFAULT 0"),
            ("selected_at", "selected_at TEXT"),
            ("updated_at", "updated_at TEXT"),
        ],
        "purchase_requests": [
            ("selected_vendor_id", "selected_vendor_id INTEGER"),
            ("selected_vendor_quote_id", "selected_vendor_quote_id INTEGER"),
            ("selected_payee_detail_id", "selected_payee_detail_id INTEGER"),
            ("approval_rescinded_at", "approval_rescinded_at TEXT"),
            ("approval_rescinded_reason", "approval_rescinded_reason TEXT"),
            ("completed_by_user_id", "completed_by_user_id INTEGER"),
            ("archived_at", "archived_at TEXT"),
            ("archived_by_user_id", "archived_by_user_id INTEGER"),
        ],
        "payment_payee_details": [
            ("approval_history_id", "approval_history_id INTEGER"),
            ("is_current", "is_current INTEGER DEFAULT 1"),
            ("replaced_at", "replaced_at TEXT"),
            ("replaced_by_user_id", "replaced_by_user_id INTEGER"),
            ("replacement_reason", "replacement_reason TEXT"),
        ],
        "payments": [
            ("transfer_type", "transfer_type TEXT"),
            ("payee_detail_id", "payee_detail_id INTEGER"),
            ("approval_history_id", "approval_history_id INTEGER"),
            ("proof_of_payment_receipt_id", "proof_of_payment_receipt_id INTEGER"),
            ("vendor_receipt_id", "vendor_receipt_id INTEGER"),
            ("currency", "currency TEXT DEFAULT 'NGN'"),
            ("payment_reference", "payment_reference TEXT"),
            ("verification_status", "verification_status TEXT DEFAULT 'Pending'"),
            ("instruction_generated_at", "instruction_generated_at TEXT"),
            ("instruction_generated_by", "instruction_generated_by INTEGER"),
            ("notification_dedupe_key", "notification_dedupe_key TEXT"),
        ],
        "receipt_records": [
            ("document_category", "document_category TEXT"),
            ("request_id", "request_id INTEGER"),
            ("payment_id", "payment_id INTEGER"),
            ("original_file_name", "original_file_name TEXT"),
            ("mime_type", "mime_type TEXT"),
            ("file_size_bytes", "file_size_bytes INTEGER"),
            ("file_checksum", "file_checksum TEXT"),
            ("ocr_status", "ocr_status TEXT DEFAULT 'Pending'"),
            ("ocr_confidence", "ocr_confidence REAL"),
            ("extracted_vendor_payee", "extracted_vendor_payee TEXT"),
            ("extracted_account_name", "extracted_account_name TEXT"),
            ("extracted_bank_name", "extracted_bank_name TEXT"),
            ("extracted_transaction_reference", "extracted_transaction_reference TEXT"),
            ("extracted_document_number", "extracted_document_number TEXT"),
            ("extracted_payment_time", "extracted_payment_time TEXT"),
            ("extracted_amount", "extracted_amount REAL"),
            ("extracted_currency", "extracted_currency TEXT"),
            ("extracted_payer", "extracted_payer TEXT"),
            ("extracted_payee", "extracted_payee TEXT"),
            ("extracted_description", "extracted_description TEXT"),
            ("extracted_tax_amount", "extracted_tax_amount REAL"),
            ("extracted_total_amount", "extracted_total_amount REAL"),
            ("original_ocr_json", "original_ocr_json TEXT"),
            ("corrected_ocr_json", "corrected_ocr_json TEXT"),
            ("discrepancy_status", "discrepancy_status TEXT DEFAULT 'Not Checked'"),
            ("discrepancy_details", "discrepancy_details TEXT"),
            ("corrected_by_user_id", "corrected_by_user_id INTEGER"),
            ("corrected_at", "corrected_at TEXT"),
            ("processing_started_at", "processing_started_at TEXT"),
            ("processing_completed_at", "processing_completed_at TEXT"),
            ("retry_count", "retry_count INTEGER DEFAULT 0"),
        ],
        "notifications": [("dedupe_key", "dedupe_key TEXT")],
        "notification_outbox": [
    ...
    schemas = [
        ...
        CREATE TABLE IF NOT EXISTS vendor_quote_items (
            ...
            FOREIGN KEY(vendor_quote_id) REFERENCES vendor_quotes(id) ON DELETE CASCADE,
        ...
        CREATE TABLE IF NOT EXISTS approval_rescissions (
            ...
            original_approver_user_id INTEGER,
            original_approver_role TEXT,
            original_approval_at TEXT,
            rescinded_by_user_id INTEGER NOT NULL,
        ...
        CREATE TABLE IF NOT EXISTS receipt_document_versions (
            ...
            corrected_ocr_json TEXT,
            created_at TEXT NOT NULL,
            UNIQUE(receipt_id, version_no),
            FOREIGN KEY(receipt_id) REFERENCES receipt_records(id) ON DELETE CASCADE
        )
        """,
    ]
    for schema in schemas:
    ...
    # Backfill legacy rows without changing their historical primary keys.
    try:
        run_query(
            """
            UPDATE vendor_quotes
            SET quotation_total=COALESCE(quotation_total, quoted_amount), currency=COALESCE(NULLIF(currency,''),'NGN'),
                request_id=COALESCE(request_id, (SELECT request_id FROM sourcing_tasks WHERE sourcing_tasks.id=vendor_quotes.sourcing_task_id)),
    ...
    try:
        run_query(
            """
            UPDATE payments SET transfer_type='Internet Bank Transfer'
            WHERE transfer_type IS NULL AND lower(COALESCE(payment_method,'')) LIKE '%bank%transfer%'
            """
        )
    except Exception:
        pass
    try:
        run_query(
            ...
            SET document_category=COALESCE(document_category, CASE WHEN lower(COALESCE(receipt_type,'')) LIKE '%vendor%' THEN 'Vendor Receipt' ELSE 'Proof of Payment' END),
                payment_id=COALESCE(payment_id, linked_payment_id), file_checksum=COALESCE(file_checksum, file_hash),
                original_ocr_json=COALESCE(original_ocr_json, ocr_json),
                ocr_status=COALESCE(NULLIF(ocr_status,''), CASE WHEN COALESCE(ocr_text,'')<>'' THEN 'Extracted' ELSE 'Pending' END)
            """
        )
    except Exception:
        pass
    try:
        run_query("UPDATE purchase_requests SET archived_at=COALESCE(archived_at, updated_at) WHERE status='Archived'")
    except Exception:
        pass
