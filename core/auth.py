...
from core.db import run_query, now_iso, log_audit, df_query, create_notification, get_conn
...
def _session_expiry_iso(remember_me: bool = False) -> str:
    ...
    if remember_me:
        expiry = datetime.now(timezone.utc) + timedelta(days=REMEMBER_ME_SESSION_DAYS)
    else:
        expiry = datetime.now(timezone.utc) + timedelta(minutes=SESSION_TIMEOUT_MINUTES)
...
def _user_session_columns() -> set[str]:
    try:
        ...
        return table_columns("user_sessions")
    except Exception:
        return set()
...
def restore_user_from_session() -> bool:
    ...
    if expires_at is not None:
        ...
        try:
            last = _parse_iso_datetime(last_seen)
            if last is None:
            ...
            if datetime.now(timezone.utc) - last > limit:
                run_query("UPDATE user_sessions SET status='Expired', logout_at=?, updated_at=? WHERE session_token=?", (now_iso(), now_iso(), token_hash))
                log_audit("SESSION_EXPIRED", "User", row.get("user_id"), "Session expired", row.get("user_id"), row.get("role"))
                return False
        except Exception:
    ...
    st.session_state["last_seen_at"] = datetime.now(timezone.utc).isoformat(timespec="seconds")
...
def rotate_persistent_session(user: dict) -> str:
    """Invalidate the current server session and issue a fresh opaque token."""
    remember_me = bool(st.session_state.get("pf_remember_me", False))
    old_token = st.session_state.get("pf_session_token")
    if old_token:
        try:
            old_hash = _session_token_hash(str(old_token))
            run_query(
                "UPDATE user_sessions SET logout_at=?, last_seen_at=?, status='Rotated', updated_at=? WHERE session_token=?",
                (now_iso(), now_iso(), now_iso(), old_hash),
            )
        except Exception:
            pass
    st.session_state.pop("pf_session_token", None)
    _clear_browser_session_token()
    token = create_persistent_session(user, remember_me=remember_me)
    if token:
...
def session_expired() -> bool:
    """Return whether the current session has expired using UTC-aware datetimes.

    PostgreSQL returns timezone-aware timestamps while legacy SQLite/session-state
    values may be timezone-naive. Normalising both through ``_parse_iso_datetime``
    prevents Python from raising when comparing aware and naive values.
    """
    ...
    if not last_seen:
        return False
    last = _parse_iso_datetime(str(last_seen))
    if last is None:
        return False
    ...
    return now_utc - last.astimezone(timezone.utc) > limit
...
def require_user() -> bool:
    ...
    if last_db_touch:
        try:
            previous_touch = _parse_iso_datetime(str(last_db_touch))
            should_touch = previous_touch is None or (now_dt - previous_touch.astimezone(timezone.utc)).total_seconds() >= 30
        except Exception:
            should_touch = True
...
def login_panel():
    ...
    st.markdown(
        ...
        @media (max-width:780px) {
            ...
            /* Final centered login layout override. */
            body:has(.pf-login-page) section.main > div {
                padding-top: 0 !important;
            }

            body:has(.pf-login-page) [data-testid="stHorizontalBlock"] {
                min-height: 100vh !important;
                align-items: stretch !important;
            }

            body:has(.pf-login-page) [data-testid="stColumn"]:nth-child(2) {
                display: flex !important;
            }

            body:has(.pf-login-page) [data-testid="stColumn"]:nth-child(2) > div {
                display: flex !important;
            ...
            body:has(.pf-login-page) .pf-login-title {
            ...
            body:has(.pf-login-page) .pf-login-subtitle {
                display: none !important;
            }

            body:has(.pf-login-page) [data-testid="stVerticalBlockBorderWrapper"]:has(.pf-login-brand) .stTextInput,
            body:has(.pf-login-page) [data-testid="stVerticalBlockBorderWrapper"]:has(.pf-login-brand) .stTextInputRootElement,
            body:has(.pf-login-page) [data-testid="stVerticalBlockBorderWrapper"]:has(.pf-login-brand) .stPasswordInput,
            body:has(.pf-login-page) [data-testid="stVerticalBlockBorderWrapper"]:has(.pf-login-brand) [data-testid="stCheckbox"] {
                max-width: 420px !important;
                margin-left: auto !important;
                margin-right: auto !important;
            }

            body:has(.pf-login-page) [data-testid="stVerticalBlockBorderWrapper"]:has(.pf-login-brand) .stButton {
                max-width: 420px !important;
                margin-left: auto !important;
                margin-right: auto !important;
            }

            body:has(.pf-login-page) [data-testid="stVerticalBlockBorderWrapper"]:has(.pf-login-brand) .stButton > button {
                width: 100% !important;
            }

            body:has(.pf-login-page) [data-testid="stVerticalBlockBorderWrapper"]:has(.pf-login-brand) p,
            body:has(.pf-login-page) [data-testid="stVerticalBlockBorderWrapper"]:has(.pf-login-brand) label {
                max-width: 420px !important;
                margin-left: auto !important;
                margin-right: auto !important;
            }
    ...
    # Native outer-column vertical alignment for login.
    left_column, right_column = st.columns([40, 60], gap="small", vertical_alignment="center")
    ...
    with right_column:
        ...
        # Structural login form width constraint.
        login_left_space, login_column, login_right_space = st.columns(
            [1.0, 2.0, 1.0],
            gap="small",
        )
...
def update_password_credentials(user_id: int, current_password: str, new_password: str, confirm_password: str) -> dict:
    """Atomically replace a password, clear the forced-change flag, and revoke old sessions.

    The Streamlit panel performs presentation and session rotation after this
    database command succeeds. Keeping the credential update transactional
    makes first-login, voluntary, and Admin-temporary-password changes use the
    same auditable security path.
    """
    conn = get_conn()
    try:
        conn.execute("BEGIN IMMEDIATE")
        row = conn.execute("SELECT * FROM users WHERE id=? AND is_active=1", (int(user_id),)).fetchone()
        if not row:
            raise ValueError("The user account is unavailable.")
        account = dict(row)
        current_hash = str(account.get("password_hash") or "")
        ...
        recent_hashes = [current_hash]
        try:
            recent_hashes.extend(
                str(r["password_hash"])
                for r in conn.execute(
                    "SELECT password_hash FROM password_history WHERE user_id=? ORDER BY created_at DESC LIMIT ?",
                    (int(user_id), PASSWORD_HISTORY_COUNT),
                ).fetchall()
            )
        except Exception:
            pass
        if any(verify_password(new_password, stored) for stored in recent_hashes if stored):
            raise ValueError("Choose a password that has not been used recently.")
        ts = now_iso()
        conn.execute(
            "INSERT INTO password_history (user_id, password_hash, created_at) VALUES (?, ?, ?)",
            (int(user_id), current_hash, ts),
        )
        conn.execute(
            ...
            (int(user_id), int(user_id), PASSWORD_HISTORY_COUNT),
        )
        new_hash = hash_password(new_password)
        conn.execute(
            "UPDATE users SET password_hash=?, must_change_password=0, failed_login_count=0, account_locked=0, updated_at=? WHERE id=?",
            (new_hash, ts, int(user_id)),
        )
        # Revoke every pre-change session. The UI immediately creates one new
        # rotated session after this transaction commits.
        conn.execute(
            "UPDATE user_sessions SET status='Password Changed', logout_at=?, last_seen_at=?, updated_at=? WHERE user_id=? AND status='Active'",
            (ts, ts, ts, int(user_id)),
        )
        conn.commit()
        account["password_hash"] = new_hash
        account["must_change_password"] = 0
        account["failed_login_count"] = 0
        account["account_locked"] = 0
...
def change_password_panel():
    ...
    if submitted:
        try:
            refreshed = update_password_credentials(int(user["id"]), current, new, confirm)
        except ValueError as exc:
            st.error(str(exc))
            return
        safe_session_user = dict(refreshed)
        safe_session_user["password_hash"] = ""
        st.session_state["user"] = safe_session_user
        rotate_persistent_session(safe_session_user)
        log_audit(
            "PASSWORD_CHANGE",
            "User",
            user["id"],
            ...
            user["id"],
            user.get("role"),
            before_values={"must_change_password": int(user.get("must_change_password") or 0)},
            after_values={"must_change_password": 0, "session_rotated": True, "obsolete_sessions_revoked": True},
        )
        st.session_state["pf_flash_success"] = "Password changed successfully. Your secure session was refreshed."
        st.rerun()
