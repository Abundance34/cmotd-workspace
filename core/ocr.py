...
def preprocess_image_variants(image):
    ...
    # Correct full-page camera rotation where Tesseract can determine it.
    # Failure is harmless: the normal variants below are still attempted.
    if pytesseract is not None and tesseract_available():
        try:
    ...
    if ImageFilter is not None:
        ...
        # Median filtering removes camera/sensor speckle without erasing text
        # edges, which improves OCR on photographed thermal receipts.
        variants.append(contrast.filter(ImageFilter.MedianFilter(size=3)))
    threshold = ImageEnhance.Contrast(gray).enhance(2.5).point(lambda p: 255 if p > 160 else 0)
    variants.append(threshold)
    # Small-angle alternatives handle common handheld skew. The scoring step
    # keeps only the best OCR output, so uncertain deskewing is never silently
    # accepted as verified data.
    variants.append(contrast.rotate(2, expand=True, fillcolor=255))
    variants.append(contrast.rotate(-2, expand=True, fillcolor=255))
    return variants
...
def parse_ocr_text(text: str, vendors_df=None) -> dict[str, Any]:
    ...
    account_name = _first_match([r"(?:account\s*name|acct\s*name|beneficiary\s*name)\s*[:#-]?\s*([A-Za-z0-9 &.,'-]{3,100})"], joined)
    ...
    payment_time = _first_match([
        r"(?:time|payment\s*time|transaction\s*time)\s*[:#-]?\s*([0-2]?\d:[0-5]\d(?::[0-5]\d)?\s*(?:AM|PM)?)",
        r"\b([0-2]?\d:[0-5]\d(?::[0-5]\d)?\s*(?:AM|PM))\b",
    ], joined)
    payer = _first_match([r"(?:payer|paid\s*by|sender|from\s*account\s*name)\s*[:#-]?\s*([A-Za-z0-9 &.,'-]{3,100})"], joined)
    payee = _first_match([r"(?:payee|beneficiary|paid\s*to|recipient|to\s*account\s*name)\s*[:#-]?\s*([A-Za-z0-9 &.,'-]{3,100})"], joined)
    narration = _first_match([r"(?:description|narration|remark|purpose)\s*[:#-]?\s*([^\n]{3,160})"], joined)
    currency = "NGN"
    if re.search(r"(?:USD|US\$|\$)", joined, flags=re.IGNORECASE): currency = "USD"
    elif re.search(r"(?:GBP|£)", joined, flags=re.IGNORECASE): currency = "GBP"
    elif re.search(r"(?:EUR|€)", joined, flags=re.IGNORECASE): currency = "EUR"
    elif re.search(r"(?:NGN|₦)", joined, flags=re.IGNORECASE): currency = "NGN"
    ...
    result["fields"] = {
        ...
        "category": category,
        "description": narration or f"{doc_type} from {matched_vendor_name or vendor_guess}".strip(),
    ...
    result["bank_details"] = {
        "bank_name": bank_name, "account_name": account_name, "account_no": account_no,
