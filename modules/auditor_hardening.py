...
def _safe_excel_download(label: str, filename: str, sheets: dict[str, pd.DataFrame], key: str) -> None:
    payload = build_excel_workbook(sheets, title=filename)
    if st.download_button(label, payload, file_name=filename, mime=excel_mime(), key=key):
...
def _ledger_filters() -> tuple[str, list[Any], dict[str, Any]]:
    ...
    if department.strip():
        ...
        clauses.append("CAST(metadata_redacted_json AS TEXT) LIKE ?")
...
def audit_dashboard() -> None:
    ...
    security = df_query("SELECT substr(occurred_at,1,10) AS event_day, COUNT(*) AS count FROM audit_events WHERE outcome IN ('Failure','Denied') OR severity IN ('High','Critical') GROUP BY 1 ORDER BY 1 DESC LIMIT 30")
    security = security.rename(columns={"event_day": "day"})
...
def expense_review_page() -> None:
    ...
    df=df_query("SELECT e.created_at, e.expense_no, e.category, e.amount, e.status, e.invoice_no, e.receipt_no, e.duplicate_warning, u.full_name created_by FROM expenses e LEFT JOIN users u ON u.id=e.requested_by ORDER BY e.created_at DESC")
    if not df.empty:
