...
def vendor_intelligence():
    ...
    perf = df_query("""
        ...
        ROUND(CAST(((COALESCE(rating,0)/5.0)*45) + (CASE WHEN COALESCE(rejection_count,0)=0 THEN 30 WHEN (COALESCE(rejection_count,0)*5)>=30 THEN 0 ELSE 30-(COALESCE(rejection_count,0)*5) END) + (CASE WHEN COALESCE(completed_orders,0)>0 THEN 25 ELSE 5 END) AS NUMERIC),1) performance_score
...
def cash_advances_tab():
    ...
    with t2:
        advances = df_query("""
            ...
            FROM cash_advances ca LEFT JOIN advance_expenses ae ON ca.id=ae.advance_id WHERE ca.status='Approved' GROUP BY ca.id, ca.advance_no, ca.employee_name, ca.amount_collected, ca.purpose, ca.due_date, ca.created_at HAVING ca.amount_collected-COALESCE(SUM(ae.amount),0) > 0 ORDER BY ca.created_at DESC
        """)
...
def analytics_dashboards():
    ...
    c5,c6 = st.columns(2)
    with c5:
        d = df_query("SELECT substr(expense_date,1,7) AS trend_month, SUM(amount) AS total FROM expenses WHERE status='Approved' GROUP BY 1 ORDER BY 1")
        d = d.rename(columns={"trend_month": "month"})
        if not d.empty: st.caption("Monthly Procurement Trend"); st.line_chart(d.set_index("month"))
    with c6:
        d = df_query("SELECT ca.employee_name, ca.amount_collected-COALESCE(SUM(ae.amount),0) AS outstanding FROM cash_advances ca LEFT JOIN advance_expenses ae ON ca.id=ae.advance_id GROUP BY ca.id, ca.employee_name, ca.amount_collected HAVING ca.amount_collected-COALESCE(SUM(ae.amount),0) > 0")
...
def answer_data_question(question: str) -> str:
    ...
    if "cash advance" in q or "not retired" in q:
        df = df_query("SELECT ca.advance_no, ca.employee_name, ca.amount_collected-COALESCE(SUM(ae.amount),0) AS balance FROM cash_advances ca LEFT JOIN advance_expenses ae ON ca.id=ae.advance_id GROUP BY ca.id, ca.advance_no, ca.employee_name, ca.amount_collected HAVING ca.amount_collected-COALESCE(SUM(ae.amount),0) > 0")
