...
from core.db import _append_audit_event_to_conn, get_conn, insert_and_get_id, json_dump, now_iso
    ...
    *,
    ...
    try:
        ...
        if existing:
            ...
            if reason:
                conn.execute(
                    "UPDATE payment_payee_details SET replaced_at=?, replaced_by_user_id=?, replacement_reason=?, is_current=1 WHERE id=?",
                    (now_iso(), actor_user_id, reason, payee_id),
                )
            action = "PAYEE_DETAILS_UPDATED"
        else:
            columns = ["purchase_request_id", *values.keys(), "created_by_user_id", "created_at"]
            placeholders = ", ".join(["?"] * len(columns))
            payee_id = insert_and_get_id(
                conn,
        ...
        conn.execute(
            "UPDATE purchase_requests SET selected_payee_detail_id=?, updated_at=? WHERE id=?",
            (payee_id, now_iso(), int(purchase_request_id)),
        )
