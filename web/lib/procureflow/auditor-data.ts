...
export type AuditorRequestRow = { id: number; requestNo: string; departmentProject: string | null; category: string | null; amount: number; status: string | null; paymentStatus: string | null; nextRole: string | null; requestedBy: string | null; requesterRole: string | null; facilityManager: string | null; procurementManager: string | null; sourceType: string | null; requestDate: string | null; updatedAt: string | null; hasPayee: boolean };
...
export async function getAuditorDashboardData(): Promise<AuditorDashboardData> {
  ...
  const [metricRows, auditRows, activityRows, workflowRows, approvalRows, notificationRows, userRows, sessionRows, requestRows, quoteRows, poRows, receivingRows, exceptionRows, paymentRows, payeeRows, gatewayRows, vendorRows, budgetRows, receiptRows] = await Promise.all([
    ...
    sql<any[]>`SELECT pr.id,pr.request_no,pr.department_project,pr.category,pr.estimated_amount,pr.status,pr.payment_status,pr.next_role,ru.full_name requested_by,ru.role requester_role,fm.full_name facility_manager,pm.full_name procurement_manager,pr.source_type,pr.request_date,pr.updated_at,ppd.id payee_id FROM purchase_requests pr LEFT JOIN users ru ON ru.id=pr.requested_by LEFT JOIN users fm ON fm.id=pr.facility_manager_user_id LEFT JOIN users pm ON pm.id=pr.assigned_procurement_manager_id LEFT JOIN LATERAL (SELECT x.id FROM payment_payee_details x WHERE x.purchase_request_id=pr.id AND COALESCE(x.is_current,TRUE)=TRUE ORDER BY x.updated_at DESC,x.id DESC LIMIT 1) ppd ON TRUE ORDER BY COALESCE(pr.request_date,pr.updated_at,pr.created_at) DESC LIMIT 500`,
  ...
  return {
    ...
    requests: requestRows.map((r)=>({id:Number(r.id),requestNo:r.request_no,departmentProject:r.department_project,category:r.category,amount:Number(r.estimated_amount||0),status:r.status,paymentStatus:r.payment_status,nextRole:r.next_role,requestedBy:r.requested_by,requesterRole:r.requester_role,facilityManager:r.facility_manager,procurementManager:r.procurement_manager,sourceType:r.source_type,requestDate:textDate(r.request_date),updatedAt:textDate(r.updated_at),hasPayee:Boolean(r.payee_id)})),
