...
import { startProcurementSourcing } from "@/lib/procureflow/procurement-sourcing-actions";
...
export async function POST(request: Request) {
  try {
    ...
    const result = await startProcurementSourcing(user, requestId, note);
    return NextResponse.json({ ok: true, result });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unable to start procurement sourcing.";
