...
import { recommendHighestScoringVendor } from "@/lib/procureflow/vendor-recommendation-actions";
...
export async function POST(request: Request) {
  try {
    ...
    const body = await request.json().catch(() => null) as { sourcingTaskId?: number } | null;
    ...
    const result = await recommendHighestScoringVendor(user, sourcingTaskId);
    return NextResponse.json({ ok: true, result });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unable to recommend a vendor.";
