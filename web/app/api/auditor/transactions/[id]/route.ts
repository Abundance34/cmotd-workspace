...
export async function GET(_request:Request,context:{params:Promise<{id:string}>}){
  try{
    ...
    const [items,workflow,approvals,payees,sourcing,quotes,pos,poItems,payments,receipts,documents,reimbursements]=await Promise.all([
      ...
      sql<any[]>`SELECT e.id,e.expense_no,e.expense_date,e.category,e.description,e.amount,e.payment_method,e.status,e.receipt_no,e.invoice_no,u.full_name requester_name,e.created_at FROM expenses e LEFT JOIN users u ON u.id=e.requested_by WHERE e.linked_request_id=${requestId} OR e.related_request_id=${requestId} ORDER BY e.created_at DESC,e.id DESC`.catch(()=>[])
    ]);
    ...
    return NextResponse.json({
      ...
      reimbursements:reimbursements.map((x:any)=>({...x,id:Number(x.id),amount:Number(x.amount||0)})),
      threadMessages:messages.map(x=>({...x,id:Number(x.id),thread_id:Number(x.thread_id)}))
    },{headers:{"Cache-Control":"private, no-store, max-age=0"}});
  }catch(error){
    return NextResponse.json({error:error instanceof Error?error.message:"Unable to load transaction detail."},{status:500,headers:{"Cache-Control":"private, no-store, max-age=0"}});
  }
}
