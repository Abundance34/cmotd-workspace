...
const PAYMENT_METHODS = new Set(["Cash", "Bank Transfer", "Card/POS", "Other"]);
...
function parseProof(input: any): PortableProof {
  ...
  if (!base64) throw new Error("Attach a valid receipt or proof of payment.");
  ...
  if (!bytes.length) throw new Error(`The supporting file '${fileName}' is empty.`);
  const checksum = createHash("sha256").update(bytes).digest("hex");
  return { fileName, mimeType, checksum, locator: `data:${mimeType};base64,${bytes.toString("base64")}`, size: bytes.length };
}

function parseProofs(raw: any, itemIndex: number) {
  const source = Array.isArray(raw?.files) ? raw.files : raw?.file ? [raw.file] : [];
  if (!source.length) throw new Error(`Item ${itemIndex + 1}: attach at least one receipt or proof of payment.`);
  if (source.length > MAX_PROOFS_PER_ITEM) throw new Error(`Item ${itemIndex + 1}: attach no more than ${MAX_PROOFS_PER_ITEM} receipt/proof files.`);
  return source.map(parseProof);
}
...
function parseManifest(locator: unknown) {
  ...
  try {
    ...
    if (!["reimbursement-batch-v2", "reimbursement-batch-v3"].includes(String(parsed?.kind || "")) || !Array.isArray(parsed.items)) return null;
...
function normalizedProofRows(item: any) {
  if (Array.isArray(item?.proofs)) {
    return item.proofs.map((proof: any, index: number) => ({
      proof_index: index,
      has_proof: String(proof?.locator || "").startsWith("data:"),
      file_name: proof?.fileName || null,
    }));
  }
  if (String(item?.locator || "").startsWith("data:")) {
    return [{ proof_index: 0, has_proof: true, file_name: item?.fileName || null }];
  }
  return [];
}
  ...
  return Array.from(grouped.values()).map((group) => {
    ...
    const items = manifest
      ? manifest.items.map((item: any) => ({
          ...
          note: item.note || null,
          payment_method: item.paymentMethod || group.payment_method || "Personal Funds",
          proofs: normalizedProofRows(item),
        }))
      : [{
          ...
          note: null,
          payment_method: group.payment_method || "Personal Funds",
          proofs: String(group.receipt_path || "").startsWith("data:") ? [{ proof_index: 0, has_proof: true, file_name: null }] : [],
        }];
...
async function candidateRequests(sql: ReturnType<typeof db>, user: { id: number; role: string }) {
  const statusSql = sql(Array.from(REIMBURSABLE_STATUSES));
  if (user.role === "Facility Manager" || user.role === "ICT") {
    return sql<any[]>`
...
export async function GET() {
  try {
    ...
    const [candidates, ownRows, reviewRows] = await Promise.all([
      ...
      sql<any[]>`
        SELECT e.id,e.expense_no,e.expense_date,e.amount,e.status,e.description,e.receipt_no,e.payment_method,e.receipt_path,e.created_at,e.requested_by,
...
export async function POST(request: Request) {
  try {
    ...
    const items: ParsedInputItem[] = rawItems.map((raw: any, index: number) => {
      ...
      const paymentMethod = clean(raw?.paymentMethod || "Bank Transfer", 40);
      ...
      if (!PAYMENT_METHODS.has(paymentMethod)) throw new Error(`Item ${index + 1}: choose a valid payment method.`);
      if (paymentMethod === "Cash" && amount > CASH_LIMIT) throw new Error(`Item ${index + 1}: cash reimbursement cannot exceed NGN ${CASH_LIMIT.toLocaleString("en-NG")}.`);
      return { requestId, amount, spendDate, reason, receiptNo, note, paymentMethod, proofs: parseProofs(raw, index) };
    });

    const cashTotal = items.filter((item) => item.paymentMethod === "Cash").reduce((sum, item) => sum + item.amount, 0);
    if (cashTotal > CASH_LIMIT) throw new Error(`The total cash portion of one reimbursement batch cannot exceed NGN ${CASH_LIMIT.toLocaleString("en-NG")}.`);
    const proofBytes = items.reduce((sum, item) => sum + item.proofs.reduce((inner, proof) => inner + proof.size, 0), 0);
    if (proofBytes > MAX_PROOF_BYTES) throw new Error("The combined receipt/proof files are too large. Keep the reimbursement batch below 2.5 MB of files.");
    ...
    const result = await sql.begin(async (tx) => {
      ...
      if (user.role !== "Procurement Manager" && assignedPmIds.length > 1) throw new Error("The selected requests belong to different Procurement Managers. Submit separate reimbursement batches for each Procurement Manager.");
      ...
      const parentMethod = methods.length === 1 ? methods[0] : "Mixed";
      ...
      const manifestItems = items.map((item) => {
        ...
        return {
          ...
          proofs: item.proofs.map((proof) => ({ fileName: proof.fileName, mimeType: proof.mimeType, checksum: proof.checksum, locator: proof.locator })),
        };
      });
      const manifest = JSON.stringify({ kind: "reimbursement-batch-v3", batchNote, cashLimit: CASH_LIMIT, cashTotal, items: manifestItems });
      const manifestForHash = manifestItems.map((item) => ({ ...item, proofs: item.proofs.map(({ locator, ...proof }) => proof) }));
      const manifestHash = createHash("sha256").update(JSON.stringify(manifestForHash)).digest("hex");
      ...
      const inserted = await tx<any[]>`
        INSERT INTO expenses (
          ...
          ${no},${earliestSpendDate},${categories.length === 1 ? categories[0] : "Multiple"},${description},NULL,${totalAmount},${parentMethod},${departments.length === 1 ? departments[0] : "Multiple Requests"},
      ...
      await appendAuditEvent(tx, {
        ...
        actorRole: user.role,
        afterValues: { total_amount: totalAmount, cash_total: cashTotal, item_count: items.length, proof_count: items.reduce((sum, item) => sum + item.proofs.length, 0), status: initialStatus, request_ids: uniqueIds },
        metadata: { document_kind: "Reimbursement", route_destination: destination, proof_manifest_hash: manifestHash, items: manifestForHash },
      ...
      if (user.role === "Procurement Manager") {
        await tx`INSERT INTO notifications (user_id,role,title,message,entity_type,entity_id,is_read,popup_shown,importance,delivery_channel,push_sent,email_sent,action_label,section_target,created_at) VALUES (NULL,'Finance','New reimbursement batch',${notificationMessage},'Expense',${reimbursementId},FALSE,FALSE,'High','in_app',FALSE,FALSE,'Review Reimbursement','Reimbursement Request',NOW())`;
      } else if (targetPmId) {
        await tx`INSERT INTO notifications (user_id,role,title,message,entity_type,entity_id,is_read,popup_shown,importance,delivery_channel,push_sent,email_sent,action_label,section_target,created_at) VALUES (${targetPmId},NULL,'Reimbursement batch requires Procurement review',${notificationMessage},'Expense',${reimbursementId},FALSE,FALSE,'High','in_app',FALSE,FALSE,'Review Reimbursement','Reimbursement Request',NOW())`;
      } else {
        await tx`INSERT INTO notifications (user_id,role,title,message,entity_type,entity_id,is_read,popup_shown,importance,delivery_channel,push_sent,email_sent,action_label,section_target,created_at) VALUES (NULL,'Procurement Manager','Reimbursement batch requires Procurement review',${notificationMessage},'Expense',${reimbursementId},FALSE,FALSE,'High','in_app',FALSE,FALSE,'Review Reimbursement','Reimbursement Request',NOW())`;
      }
      await tx`INSERT INTO notifications (user_id,role,title,message,entity_type,entity_id,is_read,popup_shown,importance,delivery_channel,push_sent,email_sent,action_label,section_target,created_at) VALUES (NULL,'Auditor','Reimbursement batch recorded',${`${no}: ${items.length} item(s), NGN ${totalAmount.toLocaleString("en-NG")}, routed to ${destination}.`},'Expense',${reimbursementId},FALSE,FALSE,'Normal','in_app',FALSE,FALSE,'Review Evidence','Expense Review',NOW())`;

      return { reimbursementId, reimbursementNo: no, status: initialStatus, destination, totalAmount, cashTotal, itemCount: items.length, requestIds: uniqueIds };
    });
...
export async function PATCH(request: Request) {
  try {
    ...
    const result = await sql.begin(async (tx) => {
      ...
      const linkedRequests = await tx<any[]>`SELECT id,request_no,assigned_procurement_manager_id FROM purchase_requests WHERE linked_expense_id=${reimbursementId} ORDER BY id`;
      ...
      await tx`UPDATE expenses SET status=${newStatus},rejection_reason=${decision === "reject" ? reviewNote : null},notes=CASE WHEN COALESCE(notes,'')='' THEN ${reviewNote} ELSE notes || E'\nProcurement review: ' || ${reviewNote} END WHERE id=${reimbursementId}`;
      for (const requestRow of linkedRequests) {
        await tx`INSERT INTO workflow_events (entity_type,entity_id,event,status,note,user_id,created_at) VALUES ('Purchase Request',${Number(requestRow.id)},${decision === "forward" ? "Reimbursement Batch Forwarded to Finance" : "Reimbursement Batch Rejected"},${newStatus},${reviewNote},${user.id},NOW())`;
      }
      await tx`INSERT INTO activity_logs (user_id,role,action,entity_type,entity_id,public_summary,private_details,visibility_scope,related_user_id,created_at) VALUES (${user.id},${user.role},${decision === "forward" ? "REIMBURSEMENT_BATCH_FORWARDED" : "REIMBURSEMENT_BATCH_REJECTED"},'Expense',${reimbursementId},${`${row.expense_no} — ${newStatus}`},${reviewNote},'workflow',${Number(row.requested_by)},NOW())`;
      await appendAuditEvent(tx, {
      ...
      await tx`INSERT INTO notifications (user_id,role,title,message,entity_type,entity_id,is_read,popup_shown,importance,delivery_channel,push_sent,email_sent,action_label,section_target,created_at) VALUES (${Number(row.requested_by)},NULL,${decision === "forward" ? "Reimbursement batch forwarded to Finance" : "Reimbursement batch rejected"},${`${row.expense_no} (${itemCount} item(s), NGN ${Number(row.amount || 0).toLocaleString("en-NG")}) is now ${newStatus}. ${reviewNote}`},'Expense',${reimbursementId},FALSE,FALSE,${decision === "forward" ? "Normal" : "High"},'in_app',FALSE,FALSE,'View Reimbursement','Reimbursement Request',NOW())`;
      if (decision === "forward") await tx`INSERT INTO notifications (user_id,role,title,message,entity_type,entity_id,is_read,popup_shown,importance,delivery_channel,push_sent,email_sent,action_label,section_target,created_at) VALUES (NULL,'Finance','Reimbursement batch forwarded by Procurement',${`${row.expense_no} from ${row.claimant_name} (${row.claimant_role}) contains ${itemCount} item(s) totalling NGN ${Number(row.amount || 0).toLocaleString("en-NG")} for ${requestNos}.`},'Expense',${reimbursementId},FALSE,FALSE,'High','in_app',FALSE,FALSE,'Review Reimbursement','Reimbursement Request',NOW())`;
      await tx`INSERT INTO notifications (user_id,role,title,message,entity_type,entity_id,is_read,popup_shown,importance,delivery_channel,push_sent,email_sent,action_label,section_target,created_at) VALUES (NULL,'Auditor','Reimbursement batch review recorded',${`${row.expense_no} changed from Pending Procurement Review to ${newStatus}.`},'Expense',${reimbursementId},FALSE,FALSE,'Normal','in_app',FALSE,FALSE,'Review Evidence','Expense Review',NOW())`;
