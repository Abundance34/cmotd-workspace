...
export function LogisticsShell({ user, data, items, securityStatus }: { user: ShellUser; data: LogisticsDashboardData; items: LogisticsPOItemRow[]; securityStatus: SecurityMigrationStatus }) {
  ...
  else if (section === "Settings") content = <article className="panel section-panel"><div className="section-icon"><ShieldCheck size={22} /></div><div><h2>Settings</h2><p>Shared account settings remain represented by the Next.js role shell while the production settings form is ported.</p></div></article>;
