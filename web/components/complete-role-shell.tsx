...
import { ApproverPaymentApprovals, ApproverPOApprovals } from "@/components/approver-operational-approvals";
...
function AuditorSection({section,data,parityData}:{section:string;data:any;parityData:ParityData}){
  if(["Transaction 360","Procurement Records","Facility / Utility Handoff Trail","Role Activity Mirrors","User 360","Exception Centre","Approval Trails","Delegated Approval Review","Notification Delivery Audit","Vendor History"].includes(section))return <AuditorReviewWorkspace section={section} data={data}/>;
  ...
  if(section==="All Activity & Evidence Ledger")return <AuditEvents rows={data?.auditEvents||[]}/>;
...
function Dashboard({role,facilityData,procurementData,approverData,financeData,logisticsData,adminData,auditorData,parityData,securityStatus}:{role:ProcureFlowRole;facilityData:any;procurementData:any;approverData:any;financeData:any;logisticsData:any;adminData:any;auditorData:any;parityData:ParityData;securityStatus:SecurityMigrationStatus}){
  ...
  else if(role==="Approver"){const approvedGatewayCount=parityData.gateways.filter((g:any)=>["Approved","Generated","Downloaded","Closed"].includes(String(g.status||""))).length;cards=[["Requests",String(approverData?.pendingRequests?.length||0),"Pending approval"],["POs",String(approverData?.pendingPOs?.length||0),"Pending PO approval"],["Payments",String(approverData?.pendingPayments?.length||0),"Payment authorization"],["Approved Gateway Passes",String(approvedGatewayCount),"Read-only management view"]];summary=`${approverData?.history?.length||0} approval-history records · gateway passes are approved by Procurement or Logistics`;}
  ...
  else if(role==="Logistics Officer"){cards=[["Awaiting Handover",String(logisticsData?.metrics?.awaitingHandover||0),"Released by Procurement"],["Gateway Reviews",String(parityData.gatewayReviewQueue.length),"Can approve gateway passes"],["Receiving",String(logisticsData?.metrics?.receivingPending||0),"Pending receiving"],["Exceptions",String(logisticsData?.metrics?.exceptions||0),"Open issues"]];summary=`${logisticsData?.receivingSlips?.length||0} receiving slips · ${logisticsData?.gatewayPasses?.length||0} gateway movements · shared Gateway Pass approval enabled`;}
