...
function confirmationFor(action: ActionName, requestNo: string) {
  if (action === "submit_approval") {
    return {
      eyebrow: "FINAL APPROVAL",
      title: "Submit request to Approver / MD?",
      description: `${requestNo} will move from Procurement review to the Approver / MD stage for final approval.`,
      confirmLabel: "Submit to Approver / MD",
    };
  }
  if (action === "sourcing") {
    return {
      eyebrow: "VENDOR SOURCING",
      title: "Start vendor quote collection?",
      description: `${requestNo} will move into Vendor Quote Collection and a sourcing task will be created.`,
      confirmLabel: "Start Sourcing",
    };
  }
  if (action === "return") {
    return {
      eyebrow: "RETURN FOR CORRECTION",
      title: "Return request to Facility Head?",
      description: `${requestNo} will be returned to the Utility / Facility Head with the correction reason entered below.`,
  ...
  return {
    ...
    description: `${requestNo} will be recorded as reviewed by Procurement.`,
...
export function ProcurementInbox({ rows }: { rows: ProcurementRequestRow[] }) {
  ...
  const confirmation = pendingAction && selected ? confirmationFor(pendingAction, selected.requestNo) : null;
  ...
  return (
    <>
            ...
            <tbody>
              {rows.map((row) => (
                <tr key={row.id} className={selected?.id === row.id ? "selected-row" : ""}>
                  ...
                  <td><button type="button" className="row-action-button" onClick={() => { setSelectedId(row.id); setMessage(null); setNote(""); setPendingAction(null); }}>Review</button></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {selected ? (
          <section className="procurement-review-card">
            ...
            <div className="procurement-review-actions">
              <button type="button" className="review-action reviewed" disabled={Boolean(busyAction)} onClick={() => requestAction("review")}><CheckCircle2 size={16} />{busyAction === "review" ? "Updating…" : "Mark Reviewed"}</button>
              <button type="button" className="review-action sourcing" disabled={Boolean(busyAction)} onClick={() => requestAction("sourcing")}><SearchCheck size={16} />{busyAction === "sourcing" ? "Opening…" : "Requires Sourcing"}</button>
              <button type="button" className="review-action return" disabled={Boolean(busyAction)} onClick={() => requestAction("return")}><CornerUpLeft size={16} />{busyAction === "return" ? "Returning…" : "Return for Correction"}</button>
              <button type="button" className="review-action approval" disabled={Boolean(busyAction)} onClick={() => requestAction("submit_approval")}><Send size={16} />{busyAction === "submit_approval" ? "Submitting…" : "Submit to Approver / MD"}</button>
            </div>
      ...
      {confirmation && selected && pendingAction ? (
        <div
          role="presentation"
          onMouseDown={(event) => {
            ...
            <div style={{ padding: "22px 24px 18px" }}>
              <div style={{ display: "flex", alignItems: "flex-start", gap: 14 }}>
                ...
                <div style={{ minWidth: 0 }}>
                  <span style={{ display: "block", marginBottom: 5, fontSize: 10.5, fontWeight: 850, letterSpacing: ".1em", color: "var(--pf-primary)" }}>{confirmation.eyebrow}</span>
                  <h3 id="procurement-confirm-title" style={{ margin: 0, fontSize: 20, lineHeight: 1.25 }}>{confirmation.title}</h3>
                  <p style={{ margin: "8px 0 0", color: "var(--pf-muted)", fontSize: 13.5, lineHeight: 1.55 }}>{confirmation.description}</p>
                </div>
              </div>

              <div style={{ marginTop: 18, padding: "12px 14px", border: "1px solid var(--pf-border)", borderRadius: 9, background: "var(--pf-surface-2)" }}>
                <span style={{ display: "block", fontSize: 11, color: "var(--pf-muted)" }}>Request</span>
                <strong style={{ display: "block", marginTop: 2, fontSize: 14.5 }}>{selected.requestNo}</strong>
                {note.trim() ? <p style={{ margin: "7px 0 0", fontSize: 12.5, color: "var(--pf-muted)" }}>Note: {note.trim()}</p> : null}
              </div>
            </div>

            <div style={{ display: "flex", justifyContent: "flex-end", gap: 10, padding: "14px 24px", borderTop: "1px solid var(--pf-border)", background: "var(--pf-surface-2)" }}>
              <button
                type="button"
