...
export function ProcurementInboxV2({ rows, approvalLimit = 0 }: { rows: ProcurementRequestRow[]; approvalLimit?: number }) {
  ...
  async function requestAction(action: ActionName) {
    ...
    if (action === "submit_approval" && lowValue) {
      setMessage({ type: "error", text: `${selected.requestNo} is within the ${money(approvalLimit)} Procurement Manager limit. Mark it Reviewed, then decide it under Low-Value Approvals.` });
      return;
    }
    ...
    const confirmation = action === "submit_approval"
      ...
      : action === "sourcing"
        ...
        : action === "return"
          ...
          : { title: lowValue ? "Mark reviewed for low-value approval?" : "Mark request as reviewed?", detail: lowValue ? `${selected.requestNo} will move to Low-Value Approvals for the Procurement Manager decision.` : `${selected.requestNo} will be recorded as reviewed by Procurement.`, label: "Mark Reviewed", tone: "primary" as const };
  ...
  async function runAction(action: ActionName) {
    ...
    try {
      ...
      const text = action === "sourcing"
        ? `${selected.requestNo} is now in Vendor Quote Collection.`
        : action === "review" && lowValue
          ? `${selected.requestNo} is reviewed and ready under Low-Value Approvals.`
  ...
  return <div className="procurement-unified-inbox">
    ...
    {selected && req && !loadingDetail ? <section className="procurement-review-card procurement-inbox-detail">
      ...
      <div className="procurement-review-actions"><button type="button" className="review-action reviewed" disabled={Boolean(busyAction)} onClick={() => void requestAction("review")}><CheckCircle2 size={16}/>{busyAction === "review" ? "Updating…" : lowValue ? "Mark Reviewed for Low-Value Approval" : "Mark Reviewed"}</button><button type="button" className="review-action sourcing" disabled={Boolean(busyAction)} onClick={() => void requestAction("sourcing")}><SearchCheck size={16}/>{busyAction === "sourcing" ? "Opening…" : "Requires Sourcing"}</button><button type="button" className="review-action return" disabled={Boolean(busyAction)} onClick={() => void requestAction("return")}><CornerUpLeft size={16}/>{busyAction === "return" ? "Returning…" : `Return to ${origin}`}</button>{!lowValue ? <button type="button" className="review-action approval" disabled={Boolean(busyAction)} onClick={() => void requestAction("submit_approval")}><Send size={16}/>{busyAction === "submit_approval" ? "Submitting…" : "Submit to Approver / MD"}</button> : null}</div>
