...
function ProcurementSection({ section, data }: { section: string; data: ProcurementDashboardData }) {
  ...
  if (section === "Vendor Recommendation") {
    return (
      <article className="panel live-section-panel">
        <div className="panel-heading"><div><h2>Vendor Recommendation</h2><p>Review the weighted supplier recommendation and route it to the correct approval authority.</p></div><span className="status-pill">{data.metrics.vendorRecommendation} pending</span></div>
        <ProcurementRecommendations tasks={data.sourcingTasks} approvalLimit={data.approvalLimit} />
      </article>
    );
  }
