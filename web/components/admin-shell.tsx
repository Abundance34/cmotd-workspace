...
import { AlertTriangle, ChevronRight, Circle, Database, LockKeyhole, LogOut, ShieldCheck, Users } from "lucide-react";
...
function UsersTable({data}:{data:AdminDashboardData}){
  ...
  return <div className="table-wrap"><table className="data-table admin-table"><thead><tr><th>User</th><th>Role</th><th>Email</th><th>Active</th><th>Locked</th><th>Failed logins</th><th>Password change</th><th>Last login</th></tr></thead><tbody>{rows.map(r=><tr key={r.id}><td><strong>{r.fullName}</strong><small>{r.username}</small></td><td>{r.role}</td><td>{r.email||"—"}</td><td>{r.active?"Active":"Inactive"}</td><td>{r.locked?"Locked":"No"}</td><td>{r.failedLoginCount}</td><td>{r.mustChangePassword?"Required":"No"}</td><td>{dateText(r.lastLoginAt)}</td></tr>)}</tbody></table></div>;
}
...
function AuditTable({data}:{data:AdminDashboardData}){
  ...
  return <div className="table-wrap"><table className="data-table admin-table"><thead><tr><th>Time</th><th>Action</th><th>Actor</th><th>Entity</th><th>Outcome</th><th>Severity</th><th>Source</th><th>Key</th></tr></thead><tbody>{rows.map(r=><tr key={r.id}><td>{dateText(r.occurredAt)}</td><td><strong>{r.action}</strong><small>{r.reason||""}</small></td><td>{r.actorUsername||"System"}<small>{r.actorRole||""}</small></td><td>{r.entityType||"—"}<small>{r.entityReference||r.entityId||""}</small></td><td>{r.outcome||"—"}</td><td>{r.severity||"—"}</td><td>{r.source||"—"}</td><td>{r.signatureKeyVersion||"—"}</td></tr>)}</tbody></table></div>;
}
function RolesPermissions({data}:{data:AdminDashboardData}){
  return <Stack><div className="admin-role-grid">{data.roles.map(r=><article key={r.id}><div><strong>{r.name}</strong><span>{r.description||"ProcureFlow role"}</span></div><b>{r.permissions.length} permissions</b><div className="admin-permission-chips">{r.permissions.map(p=><span key={p}>{p}</span>)}</div></article>)}</div><h3 className="admin-subhead">Permission catalogue ({data.permissions.length})</h3><div className="table-wrap"><table className="data-table admin-table"><thead><tr><th>Permission</th><th>Description</th></tr></thead><tbody>{data.permissions.map(p=><tr key={p.id}><td><strong>{p.name}</strong></td><td>{p.description||"—"}</td></tr>)}</tbody></table></div></Stack>;
}
function ApprovalPolicies({data}:{data:AdminDashboardData}){
  if(!data.approvalPolicies.length)return <Empty text="No approval policy settings are stored."/>;
  return <Stack><div className="admin-readonly-banner"><LockKeyhole size={16}/><div><strong>Read-only migration stage</strong><span>The production approval policy is displayed exactly from Neon. Editing is intentionally disabled until the audited Admin mutation route is ported.</span></div></div><div className="table-wrap"><table className="data-table admin-table"><thead><tr><th>Policy</th><th>Amount</th><th>Updated by</th><th>Reason</th><th>Updated</th></tr></thead><tbody>{data.approvalPolicies.map(r=><tr key={r.policyKey}><td><strong>{r.policyKey}</strong></td><td>{money(r.amount)}</td><td>{r.updatedBy||"—"}</td><td>{r.updateReason||"—"}</td><td>{dateText(r.updatedAt)}</td></tr>)}</tbody></table></div></Stack>;
}
function AvailabilityDelegation({data}:{data:AdminDashboardData}){
  return <Stack><h3 className="admin-subhead">Availability requests</h3>{data.availability.length?<div className="table-wrap"><table className="data-table admin-table"><thead><tr><th>User</th><th>Role</th><th>Status</th><th>Dates</th><th>Urgency</th><th>Review</th><th>Delegate</th><th>Reason</th></tr></thead><tbody>{data.availability.map(r=><tr key={r.id}><td>{r.userName||"—"}</td><td>{r.role||"—"}</td><td>{r.status||"—"}</td><td>{dateText(r.awayStartDate)} → {dateText(r.awayEndDate)}</td><td>{r.urgency||"—"}</td><td>{r.reviewStatus||"—"}</td><td>{r.delegateName||r.delegateRole||"—"}</td><td>{r.reason||"—"}</td></tr>)}</tbody></table></div>:<Empty text="No availability requests are pending."/>}<h3 className="admin-subhead">Delegations</h3>{data.delegations.length?<div className="table-wrap"><table className="data-table admin-table"><thead><tr><th>Primary</th><th>Delegate</th><th>Enabled</th><th>Dates</th><th>Reason</th><th>Activated by</th></tr></thead><tbody>{data.delegations.map(r=><tr key={r.id}><td>{r.primaryUser||r.primaryRole}<small>{r.primaryRole}</small></td><td>{r.delegateUser||r.delegateRole}<small>{r.delegateRole}</small></td><td>{r.enabled?"Yes":"No"}</td><td>{dateText(r.startDate)} → {dateText(r.endDate)}</td><td>{r.reason||"—"}</td><td>{r.activatedBy||"—"}</td></tr>)}</tbody></table></div>:<Empty text="No approval delegations are stored."/>}</Stack>;
}
...
export function AdminShell({user,data,securityStatus}:{user:ShellUser;data:AdminDashboardData;securityStatus:SecurityMigrationStatus}){
  ...
  else if(section==="Workflow Intervention Centre")content=<Stack><div className="admin-readonly-banner"><LockKeyhole size={16}/><div><strong>Intervention writes are locked</strong><span>Admin can inspect the live command chain here. Status overrides, rescinds and reroutes will be enabled only through a separately audited mutation route.</span></div></div><RequestsTable data={data}/></Stack>;
  else if(section==="User Management")content=<Stack><div className="admin-readonly-banner"><Users size={16}/><div><strong>User administration — read stage</strong><span>Activation, lock/unlock, reset and role-change mutations are not exposed yet.</span></div></div><UsersTable data={data}/></Stack>;
  else if(section==="Roles & Permissions")content=<RolesPermissions data={data}/>;
  else if(section==="Security & Access Management")content=<Stack><UsersTable data={data}/><h3 className="admin-subhead">Active and historical sessions</h3><SessionsTable data={data}/></Stack>;
  ...
  else if(section==="Approval Configuration")content=<ApprovalPolicies data={data}/>;
