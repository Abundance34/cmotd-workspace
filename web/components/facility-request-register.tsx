...
import { Building2, CalendarDays, ChevronRight, CircleDot, Send, X } from "lucide-react";
...
function dateTime(value: unknown) {
  if (!value) return "—";
  const d = new Date(String(value));
  return Number.isNaN(d.getTime()) ? String(value) : d.toLocaleString("en-NG");
}
...
export function FacilityRequestRegister({ rows, notifications = [], emptyText = "No Facility drafts are available." }: Props) {
  ...
  async function openRequest(row: any) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(payload?.error || "Unable to open this draft.");
      setDetail(payload);
      if (unread?.id) {
        await markNotificationRead(Number(unread.id));
        router.refresh();
      }
    } catch (error) {
      setMessage({ type: "error", text: error instanceof Error ? error.message : "Unable to open this draft." });
    } finally {
      setLoading(false);
    }
  }
  ...
  async function submitToProcurement() {
    ...
    try {
      ...
      const refresh = await fetch(`/api/facility/requests/${selectedId}`, { cache: "no-store" });
      if (refresh.ok) setDetail(await refresh.json());
      router.refresh();
    } catch (error) {
      setMessage({ type: "error", text: error instanceof Error ? error.message : "Unable to submit request." });
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return <div className="facility-register-layout">
    <div className="table-wrap facility-register-table">
      <table className="data-table">
        <thead><tr><th>Request</th><th>Department / Project</th><th>Category</th><th>Amount</th><th>Status</th><th>Updated</th><th></th></tr></thead>
        <tbody>{rows.map((row: any) => {
          const unread = unreadFor(Number(row.id));
          return <tr
            key={row.id}
            className={`clickable-request-row ${selectedId === Number(row.id) ? "selected" : ""}`}
            onClick={() => void openRequest(row)}
            onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") void openRequest(row); }}
            tabIndex={0}
          >
            <td><div className="request-register-title">{unread ? <span className="request-unread-dot" title="New activity"><CircleDot size={15}/></span> : null}<div><strong>{row.requestNo || row.request_no}</strong><small>{dateText(row.requestDate || row.request_date)}</small></div></div></td>
            <td>{row.departmentProject || row.department_project || "—"}</td>
            <td>{row.category || "—"}</td>
            <td>{money(row.estimatedAmount ?? row.estimated_amount)}</td>
            <td><span className="status-chip">{row.status || "—"}</span></td>
            <td>{dateText(row.updatedAt || row.updated_at)}</td>
    ...
    {selectedId ? <section className="facility-request-detail">
      <div className="facility-detail-head">
        <div><span>REQUEST DETAIL</span><h3>{detail?.request?.request_no || (loading ? "Loading request…" : "Request")}</h3><p>Review the draft before routing it into the Procurement Manager workflow.</p></div>
      ...
      {loading ? <div className="empty-state compact">Loading complete request detail…</div> : detail?.request ? <>
        <div className="facility-detail-summary">
          <article><Building2 size={17}/><span>Department</span><strong>{detail.request.department_project || "—"}</strong></article>
          <article><CalendarDays size={17}/><span>Required date</span><strong>{dateText(detail.request.required_date)}</strong></article>
          <article><span className="summary-currency">₦</span><span>Estimated amount</span><strong>{money(detail.request.estimated_amount)}</strong></article>
          <article><span className="summary-status-dot"/><span>Status</span><strong>{detail.request.status || "—"}</strong></article>
        </div>

        <div className="facility-detail-grid">
          <article><span>Priority</span><strong>{detail.request.priority || "—"}</strong></article>
          <article><span>Category</span><strong>{detail.request.category || "—"}</strong></article>
          <article><span>Procurement Manager</span><strong>{detail.request.procurement_manager_name || "Automatic routing pending"}</strong></article>
          <article><span>Payment readiness</span><strong>{detail.payee?.payment_readiness_status || detail.request.payment_status || "Pending Payee Details"}</strong></article>
        </div>
        ...
        <div className="facility-detail-block"><div className="facility-detail-block-title"><strong>Workflow history</strong><span>{detail.workflow?.length || 0} event(s)</span></div>
          <div className="facility-timeline">{(detail.workflow || []).map((event: any) => <div key={event.id}><span/><div><strong>{event.event}</strong><small>{event.status || ""} · {dateTime(event.created_at)}</small><p>{event.note || ""}</p></div></div>)}</div>
        </div>

        <div className="facility-detail-actions">
          <div><strong>Next step</strong><span>{SUBMITTABLE.has(String(detail.request.status || "")) ? "Review the information above, then send it to Procurement Manager." : `This request is already in ${detail.request.status || "the workflow"}.`}</span></div>
          {SUBMITTABLE.has(String(detail.request.status || "")) ? <button type="button" className="facility-submit-primary" disabled={submitting} onClick={() => void submitToProcurement()}><Send size={16}/>{submitting ? "Sending…" : "Send to Procurement Manager"}</button> : <span className="status-pill">{detail.request.status}</span>}
        </div>
      </> : null}
    </section> : null}
  </div>;
