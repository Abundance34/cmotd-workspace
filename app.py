...
def inject_shell_css():
    ...
    st.markdown(
        ...
        /* ProcureFlow global button polish. */
...
def render_sidebar_navigation(current: dict):
    ...
    # Final visual layer for the actual native Streamlit navigation buttons.
    st.markdown(
        """
        <style>
        /* Compact grouped enterprise sidebar navigation. */
        ...
        section[data-testid="stSidebar"] {
            ...
            box-shadow: 8px 0 28px rgba(15, 52, 107, .16) !important;
        }
        ...
        /* Replaced by contextual group headings. */
        .pf-sidebar-nav-label {
            display: none !important;
        }

        .pf-sidebar-section-label {
            margin: 17px 8px 7px !important;
            color: rgba(220, 233, 255, .62) !important;
            font-size: 9px !important;
            font-weight: 820 !important;
        ...
        /* Actual native Streamlit navigation button wrappers. */
        section[data-testid="stSidebar"]
        [class*="st-key-pf_nav_button_"] {
        ...
        button {
            ...
            background: transparent !important;
            box-shadow: none !important;
            color: rgba(245, 249, 255, .87) !important;
        ...
        button[data-testid="stBaseButton-primary"]:hover {
            background: #ffffff !important;
            color: #083f91 !important;
            transform: none !important;
        }

        /* Cleaner separators and supporting controls. */
        section[data-testid="stSidebar"] hr,
        ...
        section[data-testid="stSidebar"] [data-testid="stAlert"] {
            border-color: rgba(255, 255, 255, .12) !important;
            border-radius: 10px !important;
            background: rgba(3, 35, 85, .34) !important;
            color: #ffffff !important;
        }
        ...
        .pf-sidebar-account {
            ...
            background: rgba(3, 34, 82, .32) !important;
            box-shadow: inset 0 1px 0 rgba(255, 255, 255, .05) !important;
        }

        .pf-sidebar-account-avatar {
        ...
        .pf-sidebar-account-copy b {
            font-size: 12px !important;
        }

        .pf-sidebar-account-copy span {
    ...
    button_css = [
        ...
        # Streamlit has changed the generated widget-key class format across
        # releases.  Target the stable sidebar/button attributes as the source
        # of truth, then retain the key selectors only for per-item badges.
        "section[data-testid='stSidebar'] .stButton > button, section[data-testid='stSidebar'] button[kind='secondary'], section[data-testid='stSidebar'] button[data-testid='baseButton-secondary'], section[data-testid='stSidebar'] button[data-testid='stBaseButton-secondary'] { position: relative !important; width: 100% !important; min-height: 43px !important; margin: 0 !important; padding: 9px 48px 9px 12px !important; border: 1px solid transparent !important; border-radius: 10px !important; background: transparent !important; background-color: transparent !important; color: #ffffff !important; box-shadow: none !important; text-align: left !important; justify-content: flex-start !important; font-size: 12px !important; font-weight: 750 !important; line-height: 1.2 !important; opacity: 1 !important; }",
        "section[data-testid='stSidebar'] button[kind='primary'], section[data-testid='stSidebar'] button[data-testid='baseButton-primary'], section[data-testid='stSidebar'] button[data-testid='stBaseButton-primary'] { background: linear-gradient(90deg, #063d99 0%, #0b5dcc 100%) !important; background-color: #0b5dcc !important; border-color: rgba(255,255,255,.32) !important; color: #ffffff !important; box-shadow: inset 0 1px 0 rgba(255,255,255,.14), 0 6px 16px rgba(2,39,101,.24) !important; }",
        "section[data-testid='stSidebar'] .stButton > button:hover, section[data-testid='stSidebar'] button[kind='secondary']:hover, section[data-testid='stSidebar'] button[data-testid='baseButton-secondary']:hover, section[data-testid='stSidebar'] button[data-testid='stBaseButton-secondary']:hover { background: rgba(255,255,255,.15) !important; background-color: rgba(255,255,255,.15) !important; border-color: rgba(255,255,255,.18) !important; color: #ffffff !important; transform: none !important; }",
        "section[data-testid='stSidebar'] button[kind='primary']:hover, section[data-testid='stSidebar'] button[data-testid='baseButton-primary']:hover, section[data-testid='stSidebar'] button[data-testid='stBaseButton-primary']:hover { background: linear-gradient(90deg, #063d99 0%, #0b5dcc 100%) !important; background-color: #0b5dcc !important; border-color: rgba(255,255,255,.38) !important; color: #ffffff !important; transform: none !important; }",
        "section[data-testid='stSidebar'] .stButton > button p, section[data-testid='stSidebar'] .stButton > button span, section[data-testid='stSidebar'] button[data-testid^='baseButton-'] p, section[data-testid='stSidebar'] button[data-testid^='baseButton-'] span, section[data-testid='stSidebar'] button[data-testid^='stBaseButton-'] p, section[data-testid='stSidebar'] button[data-testid^='stBaseButton-'] span { color: #ffffff !important; opacity: 1 !important; font-weight: 750 !important; text-align: left !important; }",
    ]
    for index, section in enumerate(sections):
        button_key = f"pf_nav_button_{state_key}_{index}"
        safe_button_key = button_key.replace("_", "-")
        # Match the widget-key class as a complete class token. A substring
        # selector for index ``1`` also matches ``10`` through ``19``, which
        # made one Purchase Requests badge appear on unrelated later tabs.
        selector_parts = [
    ...
    # Contextual headings improve navigation hierarchy without changing
    # section names, routing, widget keys, notification logic, or permissions.
    def sidebar_group_for(section_name: str) -> str:
        section_text = str(section_name or "").casefold()

        if "setting" in section_text:
            return "Account"
    ...
    for index, section in enumerate(sections):
        group_name = sidebar_group_for(section)

        if group_name != current_group:
            st.markdown(
                (
                    '<div class="pf-sidebar-section-label">'
                    + escape(group_name)
                    + "</div>"
                ),
                unsafe_allow_html=True,
        ...
        # The exact-key CSS pseudo-element above renders the single badge.
        # Do not repeat the count in the button label.
        ...
        if st.button(
            ...
            _mark_sidebar_section_notifications_read(
                current,
                section,
                list(sections),
            )
...
def maybe_play_notification_sound(current: dict):
    """
    Plays a soft browser notification sound when the current user's unread
    notification count increases during the session.

    Browser autoplay rules still apply, so the sound works best after the
    ...
    enabled = str(
        os.environ.get(
            "PROCUREFLOW_NOTIFICATION_SOUND_ENABLED",
            "1",
        )
    ...
    state_key = f"pf_unread_notification_count_{user_id}"
    previous_count = st.session_state.get(state_key)
    st.session_state[state_key] = unread_count

    # Do not play on the user's first page load; only on increases.
    if previous_count is None or unread_count <= previous_count:
        return
    ...
    st.markdown(
        ...
        (function () {
            ...
            try {
                ...
                // Browser audio policy may block playback; fail silently.
            }
        })();
        </script>
        """,
...
def main():
    ...
    maybe_play_notification_sound(current)
